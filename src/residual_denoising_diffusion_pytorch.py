...
class UnetRes(nn.Module):
    ...
    def forward(self, x, time, x_self_cond=None):
        if self.share_encoder == 1:
            ...
            return self.unet0(x, time, x_self_cond=x_self_cond), self.unet1(
                x, time, x_self_cond=x_self_cond
            )
        elif self.share_encoder == -1:
...
class ResidualDiffusion(nn.Module):
    ...
    def predict_noise_from_res(self, x_t, t, x_input, pred_res):
        return (
            x_t - x_input - (extract(self.alphas_cumsum, t, x_t.shape) - 1) * pred_res
    ...
    def model_predictions(
        self, x_input, x, t, x_input_condition=0, x_self_cond=None, clip_denoised=True
    ):
    ...
    def p_sample_loop(self, x_input, shape, last=True):
        ...
        for t in tqdm(
            ...
            img, x_start = self.p_sample(x_input, img, t, x_input_condition, self_cond)
    ...
    def ddim_sample(self, x_input, shape, last=True):
        ...
        for time, time_next in tqdm(time_pairs, desc="sampling loop time step"):
            ...
            if type == "use_pred_noise":
                ...
                img = (
                    sqrt_betas2_cumsum_next_minus_sigma2_divided_betas_cumsum * img
                    + (1 - sqrt_betas2_cumsum_next_minus_sigma2_divided_betas_cumsum)
                    * x_start
                    + (
                        alpha_cumsum_next
                        - alpha_cumsum
        ...
        if self.self_condition and random.random() < 0.5:
            with torch.no_grad():
                x_self_cond = self.model_predictions(
                    x_input, x, t, x_input_condition if self.input_condition else 0
                ).pred_x_start
                x_self_cond.detach_()
...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        # RAdam
        # self.opt = RAdam(diffusion_model.parameters(), lr=train_lr, betas=adam_betas)
    ...
    def sample(self, milestone, last=True, FID=False):
        ...
        with torch.no_grad():
            ...
            all_images_list = show_x_input_sample + list(
                self.ema.ema_model.sample(x_input_sample, batch_size=batches, last=last)
            )
            ...
            if FID:
                for i in range(batches):
                    ...
                    utils.save_image(
                        all_images_list[0][i].unsqueeze(0),
